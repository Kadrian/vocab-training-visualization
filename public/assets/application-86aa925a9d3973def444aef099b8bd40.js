...
(function() {
  var allowedTrials, nextWord, removeRandom, selectRandom, setupSliderInput, startTraining, stopTraining, submitAnswer, updateLabel;
  ...
  nextWord = function() {
    ...
    return $('#question').html(eng);
  };

  removeRandom = function(array) {
    var idx;
  ...
  submitAnswer = function(answer) {
    var jap, _i, _len, _ref;
    _ref = window.word["jap"];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      jap = _ref[_i];
      if (answer === jap) {
        console.log("CORRECT");
    ...
    $('#answer-form').addClass('has-error');
    window.trials++;
    if (window.trials > allowedTrials) {
      $('#answer-form').removeClass('has-error');
      return nextWord();
    }
  };

  updateLabel = function(range) {
    return $('#numwords-label').html(Math.abs(range[1] - range[0]) + " words");
  };

  setupSliderInput = function(numVocab) {
    var initMax, initMin, max, min;
    min = 0;
    max = numVocab;
    initMin = numVocab / 2;
    initMax = numVocab;
    ...
    $('#slider').on('slide', function() {
      var range;
      range = $(this).data('slider').getValue();
      $('#min').val(range[0]);
      $('#max').val(range[1]);
      return updateLabel(range);
    });
    $('#min').val(initMin);
    $('#min').on('input', function() {
      var range;
      range = $('#slider').data('slider').getValue();
      range[0] = parseInt($(this).val());
      $('#slider').slider('setValue', range);
      return updateLabel(range);
    });
    $('#max').val(initMax);
    return $('#max').on('input', function() {
      var range;
      range = $('#slider').data('slider').getValue();
      range[1] = parseInt($(this).val());
      $('#slider').slider('setValue', range);
      return updateLabel(range);
    });
  };
  ...
  $(function() {
    $.ajax('/exercise/vocab', {
      ...
      success: function(data, textStatus, jqXHR) {
        ...
        if (numVocab > 10) {
          return setupSliderInput(numVocab);
        } else {
          return console.log("ERROR: You need at least 10 Words in your vocabulary.");
        }
      }
    });
    ...
    return $('#answer').keyup(function(e) {
      if (e.keyCode === 13) {
        return submitAnswer($(this).val());
      }
    });
  });
